...
- 작업이 일정 이상 시간을 요구할 것으로 예측되면 서브 에이전트를 생성하여 병렬로 실행할 수 있다. 독립적인 작업 단위(서로 의존하지 않는 모듈 구현, 병렬 가능한 조사 등)를 식별하여 분배한다.
...
다음 순서대로 실행한다. 각 리뷰의 수정이 커밋된 후 다음 리뷰를 진행하여, 항상 최신 코드를 대상으로 분석한다.

1. `/self-review`로 내부 심층 분석을 실행하고, 버그·계약 불일치·설계 위반을 확인하여 반영한다.
2. `/codex-review`로 외부 리뷰를 실행하고, 지적 사항을 평가하여 타당한 항목은 반영한다.
3. `/security-review`로 보안 관점 분석을 실행하고, 취약점·민감 정보 노출·권한 문제를 확인하여 반영한다.

- 세 리뷰 모두 즉시 반영 항목이 0건(clean pass)이 될 때까지 반복한다.
- 리뷰 결과를 사용자에게 보고한다.
