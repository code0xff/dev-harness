---
name: next-iteration
description: 현재 iteration 완료 후 다음 iteration의 service_goal과 workstream을 정의하고 autopilot을 재트리거하는 workflow
user-invocable: true
---

# Next Iteration $ARGUMENTS

현재 iteration이 delivered 상태일 때 다음 iteration을 정의하고 autopilot을 재실행한다.
각 iteration은 완료 시 사용자 관점에서 의미 있는 서비스 완결 상태를 달성해야 한다.
...
- `delivered`가 아니면: 현재 iteration이 아직 완료되지 않았음을 사용자에게 알리고 중단한다.
  ...
  - 강제로 다음 iteration을 정의하려면 사용자가 명시적으로 확인해야 한다.
- `delivered`면: 다음 단계로 진행한다.

## 3. 다음 iteration 정의

사용자와 대화하여 다음 iteration을 정의한다.

### 3-1. service_goal 확정

이 iteration이 완료된 후 **사용자가 할 수 있는 것**을 한 문장으로 기술한다.

- 사용자 관점의 완결된 행동이어야 한다 (예: "사용자가 결제하고 영수증을 받을 수 있다")
- 기술적 구현 설명이 아니라 서비스 가치 기술이어야 한다
- 이 iteration만 완료해도 독립적으로 릴리스 가능한 상태여야 한다
...
- 과도하게 많으면 다음 iteration으로 일부를 분리한다

## 5. roadmap append

검증이 완료되면 roadmap에 새 iteration 블록을 추가한다.
...
`docs/execution-plan.md`에도 새 iteration plan 섹션을 추가한다.
...
autopilot 실행:
```bash
.claude/hooks/run-autopilot.sh start "<next_iteration_goal>"
```

`<next_iteration_goal>`은 다음 형식으로 구성한다:
```
<service_goal> [iteration=<N>; execution_mode=plan_all_workstreams_then_build_sequentially; verification_mode=acceptance_first]
```
