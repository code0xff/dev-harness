...
description: 구현 완료 후 보안 관점 반복 리뷰 — 수정할 항목이 없을 때까지 분석-수정 사이클을 반복한다
user-invocable: true
---
...
수정이 1건이라도 발생하면 (4. 수정 적용에서 이미 commit됨) 변경 사항을 다시 수집하고 보안 분석을 재실행한다.
...
- 이전 iteration의 수정이 새로운 보안 문제를 만들었는지에 집중하여 분석한다.
...
### 즉시 반영한 항목
(각 항목: iteration 번호, 파일, 취약점 유형, 변경 내용, 발견 근거. 없으면 `없음`)
